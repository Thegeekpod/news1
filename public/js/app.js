...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function getYoutubeEmbedUrl(url) {
    const regExp = /^.*(navigator\.|\/\/)?(youtu\.be\/|v\/|u\/\w\/|embed\/|watch\?v=|\&v=)([^#\&\?]*).*/;
  ...
  if (videoCards.length > 0 && videoModal && videoIframe && closeModal) {
  ...
  // ===== TOP 10 LIVE STOCK MARKET WIDGET =====
  ...
  if (stockWidget) {
    ...
    const spotlightCard = document.getElementById('stock-spotlight-card');
    ...
    async function fetchStockMarketData(isManual = false) {
      ...
      try {
        ...
        if (data.status === 'success') {
          ...
          // Start or update Spotlight Auto-Rotation
          if (!spotlightInterval && allStocks.length > 0) {
        ...
        if (stockListContainer && allStocks.length === 0) {
          stockListContainer.innerHTML = '<div style="text-align:center; padding:15px; color:var(--text-muted); font-size:0.8rem;">বাজার তথ্য লোড করা সম্ভব হয়নি। আবার চেষ্টা করুন।</div>';
        }
      } finally {
        if (stockRefreshBtn) {
