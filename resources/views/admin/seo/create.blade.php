...
<div class="admin-card" style="padding: 0; overflow: hidden; border: 1px solid var(--border-color); border-radius: 8px; box-shadow: 0 4px 16px rgba(0,0,0,0.1);">
  ...
  <div style="background: #1877f2; color: #ffffff; padding: 14px 20px; font-weight: 700; font-size: 1.1rem; border-top-left-radius: 8px; border-top-right-radius: 8px;">
    SEO Details
  </div>

  <form action="{{ route('admin.seo.store') }}" method="POST" style="padding: 24px;">
    @csrf
    ...
    <div class="form-group" style="margin-bottom: 22px;">
      ...
      <input type="text" name="page_url" id="page_url" class="form-control" placeholder="e.g. /about or / or /category/sports" value="{{ old('page_url') }}" required
    ...
    <div class="form-group" style="margin-bottom: 22px;">
      ...
      <input type="text" name="meta_title" id="meta_title" class="form-control" placeholder="Enter meta title for search engines..." value="{{ old('meta_title') }}"
        style="width: 100%; padding: 10px 14px; border-radius: 6px; border: 1px solid var(--border-color); background: var(--bg-surface-subtle); color: var(--text-primary); font-size: 0.95rem;">
    </div>
    ...
    <div class="form-group" style="margin-bottom: 22px;">
      ...
      <textarea name="meta_description" id="meta_description" class="form-control" rows="4" placeholder="Enter meta description for search engines..."
        style="width: 100%; padding: 10px 14px; border-radius: 6px; border: 1px solid var(--border-color); background: var(--bg-surface-subtle); color: var(--text-primary); font-size: 0.95rem; font-family: inherit;">{{ old('meta_description') }}</textarea>
    </div>
    ...
    <div class="form-group" style="margin-bottom: 26px;">
      ...
      <textarea name="other_tags" id="other_tags" class="form-control" rows="5" placeholder="<script>...</script> or <meta ...>"
        style="width: 100%; padding: 10px 14px; border-radius: 6px; border: 1px solid var(--border-color); background: var(--bg-surface-subtle); color: var(--text-primary); font-size: 0.9rem; font-family: monospace;">{{ old('other_tags') }}</textarea>
      <small style="display: block; margin-top: 5px; color: var(--text-muted); font-size: 0.82rem;">
    ...
    <div style="display: flex; gap: 12px; align-items: center;">
      <button type="submit" class="btn-admin btn-admin-primary" style="padding: 10px 24px; font-weight: 700;">
        <i class="fas fa-save"></i> Save SEO Details
      </button>
